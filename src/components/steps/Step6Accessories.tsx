  ...
  return (
    <div className="animate-fadein">
      ...
      <div className="max-w-5xl mx-auto px-6 py-6">
        ...
        {/* Шапка с поставщиком и кнопкой пересчёта */}
          ...
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ backgroundColor: currentMeta.color, boxShadow: `0 0 10px ${currentMeta.color}55` }}
              >
                {currentMeta.name.slice(0, 2).toUpperCase()}
              </div>
              <div>
                <div className="text-sm font-bold text-[var(--text-primary)]">{currentMeta.name}</div>
                <div className="flex items-center gap-2">
                  <span className={state.voltage === 48 ? 'badge-48v' : 'badge-220v'}>
                    {state.voltage ?? '?'}В
                  </span>
                  {currentMeta.badge && (
                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-amber-500/20 text-amber-400 font-bold">{currentMeta.badge}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="text-xs border border-[var(--border)] text-[var(--text-muted)] hover:border-[var(--neon)] hover:text-[var(--neon)] px-3 py-1.5 rounded-lg transition-all disabled:opacity-40"
              >
                Пересчитать
              </button>
            </div>
